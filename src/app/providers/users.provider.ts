...
import { LoginResult, SignupData, SignupResult, Profile, FbLoginData, GpLoginData, AppleLoginData } from '@models/users';
...
export class UsersService {
  ...
  login(code: string): Observable<LoginResult> {
    ...
    return this.base.post<LoginResult>(this.endpoints.login, "code="+code);
  }
  ...
  check_provider(data: any) {


    this.logger.log(this.TAG, 'signup');

    return this.base.post<any>(this.endpoints.check_provider, data);
  }
  ...
  profile(token) {
    ...
    return this.base.post<Profile>(this.endpoints.profile, { token: token }, token);
  }
}
